...
if __name__ == "__main__":
    ...
    if not os.path.exists(profile_dir):
        ...
        if os.path.exists(profile_dir + save_file):
            with open(profile_dir + save_file, "rb") as fp:
                steam_id_set, group_id_set = pickle.load(fp)
    profile_sum_now = 0
    save_step_now = 0
    while len(steam_id) != 0 and profile_sum_now != profile_sum:
        save_step_now += 1
        if save_step_now == save_step:
            save_step_now = 0
            with open(profile_dir + save_file, "wb") as fp:
                pickle.dump((steam_id_set, group_id_set), fp)
        while len(steam_id_set) == 0:
            gid = group_id_set.pop()
            member_list = group_memberslist(gid)
            if member_list is None:
                continue
            for member in member_list:
                steam_id_set.add(member)
        sid = steam_id_set.pop()
    ...
    print("Done")
    print("Profile: %d, SteamID: %d, GroupID: %d" % (profile_sum_now, len(steam_id_set), len(group_id_set)))